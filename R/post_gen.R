...
#' Generic function to post data into IO databases
#'
...
#'
#' @param endpoint `character`. Name of the io data object destination.
#' Corresponds to the name of a within IO Postgresql database, stored within
...
#' improve rapidity
#' @param .header `list` Optional. Named list of key values to be added to the
#' request header.
#' @return NULL
...
post_gen <- function(
    ...
    url <- httr::modify_url(IO_API_HOST(),
        path = paste(
            httr::parse_url(IO_API_HOST())$path,
            endpoint, sep = "/"))
